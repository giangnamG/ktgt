...
def profile():
    ...
    if 'user_migrate' in username:
        return "Permission Deny", 403
    ...
    html_template = f"""
        ...
        <body class="container mt-5">
            ...
            <div class="profile-info">
                ...
                <p><strong>🎂 secret_key:</strong> {user.get('secret_key', '')}</p>
                <p><strong>🎂 cover_image_name:</strong> {user.get('cover_image_name', '')}</p>
...
def backup_account(username):
    ...
    user.cover_image_name = random_image.split('/')[-1]  # Cập nhật cover_image_name
    db.session.commit()  # Lưu thay đổi vào cơ sở dữ liệu
...
def activate():
    if request.method == 'GET':
        ...
        # DCT Decrypt to check secret_key
            ...
            # return f"{COVER_IMAGE_FILEPATH}{cover_image_name} - {filename}"       
            DCT = DCTApp(
                message="",
                cover_image_name="",
                stego_image_name=f"../{filename}"
            )
